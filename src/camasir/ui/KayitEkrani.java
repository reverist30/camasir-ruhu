...
public class KayitEkrani extends JDialog {
	public KayitEkrani() {
		...
		this.setModal(true); // arkada acik olan ekrani dondurur.
		...
		btnKayit.addActionListener(e -> {
			...
			if (ogrTC.equals("11111111111")) {
				lblKayit[4].setText("Hata! Bu öğrenci sistemde kayıtlıdır.");
				return;
			}
			int katSayisi = 9;
			int herKatOda = 30;
			...
			try {
				if (ogrOdaNo.toLowerCase().startsWith("z")) {
					int sayi = Integer.parseInt(ogrOdaNo.substring(1)); // oda numarasinin ilk basamagi atilarak
																		// integera cevrildi
				...
				lblKayit[4].setText("Hata! Oda numarasını dogru formatta yazınız!");
				return;
			...
			try {
				Ogrenci yeniKayit = new Ogrenci(ogrIsim, ogrTC, secimKutusu.getSelectedItem() + "-" + ogrOdaNo,
						ogrTelNo, 0);
				// Safanin kayit fonksiyonu gelecek
